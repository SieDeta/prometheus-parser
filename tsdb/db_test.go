...
import (
	...
	"github.com/zzylol/prometheus-sketch-VLDB/prometheus-sketches/config"
	...
	"github.com/zzylol/prometheus-sketch-VLDB/prometheus-sketches/tsdb/index"
	"github.com/zzylol/prometheus-sketch-VLDB/prometheus-sketches/tsdb/record"
	"github.com/zzylol/prometheus-sketch-VLDB/prometheus-sketches/tsdb/tombstones"
	"github.com/zzylol/prometheus-sketch-VLDB/prometheus-sketches/tsdb/tsdbutil"
	"github.com/zzylol/prometheus-sketch-VLDB/prometheus-sketches/tsdb/wlog"
...
func TestMain(m *testing.M) {
	...
	goleak.VerifyTestMain(m, goleak.IgnoreTopFunction("github.com/zzylol/prometheus-sketch-VLDB/prometheus-sketches/tsdb.(*SegmentWAL).cut.func1"), goleak.IgnoreTopFunction("github.com/zzylol/prometheus-sketch-VLDB/prometheus-sketches/tsdb.(*SegmentWAL).cut.func2"))
}
...
// https://github.com/zzylol/prometheus-sketch-VLDB/prometheus-sketches/issues/7548
...
// See https://github.com/zzylol/prometheus-sketch-VLDB/prometheus-sketches/issues/5105
...
// https://github.com/zzylol/prometheus-sketch-VLDB/prometheus-sketches/issues/9846
// https://github.com/zzylol/prometheus-sketch-VLDB/prometheus-sketches/issues/9859
...
// Regression test for https://github.com/zzylol/prometheus-sketch-VLDB/prometheus-sketches/pull/6514.
...
// TestCompactHeadWithDeletion tests https://github.com/zzylol/prometheus-sketch-VLDB/prometheus-sketches/issues/11585.
...
// Tests https://github.com/zzylol/prometheus-sketch-VLDB/prometheus-sketches/issues/10291#issuecomment-1044373110.
...
// Regression test for https://github.com/zzylol/prometheus-sketch-VLDB/prometheus-sketches/pull/13754
	...
	for i := 0; i < scrapeCount; i += 100 {
		...
		for len(lbls) > 0 {
			...
			go func() {
				...
				if err != nil {
					// exitWithError(err)
					fmt.Println("ingestScrapesShardZipf err", err)
				}
				total.Add(n)
			}()
		}
		...
		fmt.Println("tmp throughput:", i, total.Load(), scrapeCount, elapsed.Seconds(), float64(total.Load())/(elapsed.Seconds()))
	}
	fmt.Println("ingestion completed")
...
func ingestScrapesShardZipf(st *DB, lbls []labels.Labels, scrapeCount int, baset int64, z *rand.Zipf, promcache *promsketch.PromSketches) (uint64, error) {
	...
	for i := 0; i < scrapeCount; i++ {
		...
		for j, s := range scrape {
			...
			go func() {
				defer wg.Done()
				_ = promcache.SketchInsert(scrape[j].labels, ts, float64(scrape[j].value[i]))
			}()

			ref, err1 := app.Append(ref, s.labels, ts, float64(scrape[j].value[i]))
			if err1 != nil {
				panic(err1)
			}
		...
		if err := app.Commit(); err != nil {
			fmt.Println("tsdb commit error; return")
			return total, err
		}
...
func TestDBInsertionThroughputZipf(t *testing.T) {
	...
	scrapeCountBatch := 3000000 // seconds, 12 hours
	num_ts := flagvar
	promcache := promsketch.NewPromSketches()
	...
	start := time.Now()
	total, _ := ingestScrapesZipf(st, lsets, scrapeCountBatch, promcache)

	since := time.Since(start)

	throughput := float64(total) / float64(since.Seconds())
	t.Log(num_ts, since.Seconds(), throughput)
