...
import (
	...
	"github.com/zzylol/prometheus-sketch-VLDB/prometheus-sketches/util/stats"
...
// see https://github.com/zzylol/prometheus-sketch-VLDB/prometheus-sketches/issues/8949.
		...
		for i, s := range selVS.Series {
			...
			for ts, step := ev.startTimestamp, -1; ts <= ev.endTimestamp; ts += ev.interval {
				...
				if ts == ev.startTimestamp || selVS.Timestamp == nil {
					...
					lookup := ev.sketchCache.LookUpAndUpdateWindow(selVS.Series[i].Labels(), e.Func.Name, mint, maxt-100)
					if total_print == 20 {
						fmt.Println("[engine] funcname, mint, maxt=", e.Func.Name, mint, maxt, maxt-100, maxt-mint)
						fmt.Println("sketch instance coverage, lookup=", lookup)
						fmt.Println(ev.sketchCache.PrintCoverage(selVS.Series[i].Labels(), e.Func.Name))
					}
					...
					if lookup {
						...
						*/
						...
						if ev.isrule {
							...
							t_sketch := time.Now()
							err := ev.sketchCache.NewSketchCacheInstance(selVS.Series[i].Labels(), e.Func.Name, (maxt-mint)*4, int64(float64(maxt-mint)/float64(ev.ScrapeInterval))*4, 100000)
...
func (ev *evaluator) rangeEvalTimestampFunctionOverVectorSelector(vs *parser.VectorSelector, call FunctionCall, e *parser.Call) (parser.Value, annotations.Annotations) {
	...
	return ev.rangeEval(nil, func(v []parser.Value, _ [][]EvalSeriesHelper, enh *EvalNodeHelper) (Vector, annotations.Annotations) {
		if vs.Timestamp != nil {
			...
			// See https://github.com/zzylol/prometheus-sketch-VLDB/prometheus-sketches/issues/8433.
